...
def get_config():
  ...
  config.prune_global = True
  config.prune_percentage = .5
  config.quant = ml_collections.ConfigDict()
  config.quant.weight = prune

  # those are not meaningful but place holder for the system to work.
  config.quant.bits = 999
  config.quant.init_fn = lambda x: x
  config.quant.g_scale = 999
